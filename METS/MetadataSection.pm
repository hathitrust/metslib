...
sub set_md_ref {
    ...
    $self->{'mdref'} = METS::createElement("mdRef",METS::copyAttributes(\%attrs,
	    'ID',@METS::LOCATION,@METS::METADATA,@METS::FILECORE,'LABEL','XPTR'));

    METS::setXLink($self->{'mdref'},$attrs{'xlink'}) if defined($attrs{'xlink'});
...
sub set_data {
    ...
    $self->{'mdwrap'} = METS::createElement("mdWrap",METS::copyAttributes(\%attrs,
	    'ID',@METS::METADATA,@METS::FILECORE,'LABEL'));
	...
	my $binDataNode = METS::createElement("binData",undef,$encoded_data);
    }
...
sub set_xml_file {
    ...
    $self->set_data($parsed_xml->documentElement(),
	mimetype => "text/xml",
	@_);
}
...
sub to_node {
    my $self = shift;
    my $node = METS::createElement($self->{'element_name'},$self->{'attrs'});

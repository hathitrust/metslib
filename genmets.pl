...
my $voldiv = $struct_map->add_div( type => 'volume' );
for ( my $i = 0 ; $i < $maxid ; i++ ) {
    my $imgnum = sprintf( "%08d", $i );
    my $pagediv = $voldiv->add_div(
        files => [
            $img_filegroup->get_id("$imgnum.jp2"),
            $ocr_filegroup->get_id("$imgnum.txt"),
            $xml_filegroup->get_id("imgnum.xml")
        ],
        order => $i,
        type  => "page",
        label => "PAGETAG"
...
print $mets->get_mets()->toString(1);
